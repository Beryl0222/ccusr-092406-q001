...
- **只追加调整**：撤回、重复确认、复议、判决变化均产生追加决定，原决定原样保留；
  追加决定同样走审核（及必要时会签），驳回或在途的调整不影响余额。
- **调整与资金边界**：追加决定生效**只改变当前核准上限（应付上限）**，不直接出款：
  - 未付款时降额：不产生任何资金记录，仅压低后续可付上限；
  - 部分付款后降额：追回额 = `max(0, 实际已付 − 新核准额)`，不超过已付超额；
  - 升额：不绕过支付岗位自动补付，只形成可付余额，由支付执行人逐笔支付，
    跨会签线的升额仍须财政会签后才放开上限；
  - 支付与调整共用全局串行锁与递增顺序号，并发只能得到一个可解释顺序。
- **追加式资金谱系**：原决定、历次调整、每笔支付/追回均原样保留、不就地冲销；
  调整记录写入 `clawback_amount`，每笔流水带 `ledger_seq` 全局顺序号。
...
| `GET /cases/{id}/explain` | 逐人说明：资格/待办审批/核准金额/已付金额/可付余额/待追回/调整沿革 |
...
npm test                     # 契约 + 领域规则 + HTTP 端到端 + 调整/并发边界，共 47 项
```
